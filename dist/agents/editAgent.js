...
export class EditAgent {
    ...
    async execute(input, context) {
        const { segments, platforms, style } = input;
                ...
                try {
                    ...
                    if (ffmpegAvailable && segment.videoPath) {
                        try {
                            ...
                            logger.warn(`FFmpeg failed on source clip: ${err.message}`);
                        }
                    }
                    ...
                    if (!finalPath || !existsSync(finalPath)) {
                        try {
                            ...
                            logger.warn(`VideoGenerator also failed: ${genErr.message}`);
                        }
                    }
                    const aspectRatios = { tiktok: '9:16', instagram: '9:16', youtube: '16:9', twitter: '1:1' };
                    editedVideos.push({
                        id: uuidv4(), segmentId: segment.id, platform: platform,
                    ...
                    logger.warn(`Edit failed for ${platform}: ${err.message}`);
                }
            }
        }
