...
export class FFmpegService {
    ...
    async extractSegment(options) {
        ...
        if (exitCode !== 0)
            throw new Error(`FFmpeg segment extraction failed`);
    ...
    async formatForPlatform(inputPath, outputPath, platform) {
        const dimensions = {
            tiktok: [1080, 1920],
            instagram: [1080, 1920],
            youtube: [1920, 1080],
        ...
        if (exitCode !== 0)
            throw new Error(`FFmpeg format failed for ${platform}`);
        return { outputPath, width: w, height: h };
    }
    async burnCaptions(inputPath, outputPath, captions) {
        ...
        if (exitCode !== 0)
            throw new Error('FFmpeg caption burn failed');
        return outputPath;
    }
    async addHookOverlay(inputPath, outputPath, hookText) {
        ...
        if (exitCode !== 0)
            throw new Error('FFmpeg hook overlay failed');
        return outputPath;
    }
