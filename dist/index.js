...
program
    ...
    const validAgents = ['research', 'clip', 'edit', 'post', 'finance'];
    if (options.agent && !validAgents.includes(options.agent)) {
    ...
    try {
        ...
        table.push(['Status', statusColor(result.status)], ['Run ID', result.id], ['Duration', duration], ['Tasks', result.tasks.length.toString()], ['Niche', result.niche]);
        ...
        if (options.loop) {
            ...
            while (true) {
                ...
                try {
                    ...
                    loopSpinner.fail(chalk.red(`Run #${runCount} failed: ${err.message}`));
        ...
        runSpinner.fail(chalk.red(`Pipeline failed: ${error.message}`));
        process.exit(1);
    }
});
// ── STATUS command ──
program
    ...
    if (run) {
        ...
        table.push(['Run ID', run.id], ['Status', statusColor(run.status)], ['Niche', run.niche], ['Started', dayjs(run.startedAt).format('YYYY-MM-DD HH:mm:ss')], ['Duration', duration], ['Tasks Total', run.tasks.length.toString()], ['Tasks Complete', run.tasks.filter(t => t.status === 'complete').length.toString()], ['Tasks Failed', run.tasks.filter(t => t.status === 'failed').length.toString()]);
        console.log(chalk.blue('\n  Current Pipeline:'));
        console.log(table.toString());
        if (run.errorMessage) {
...
program
    ...
    table.push(['Total Tokens Used', report.totalTokens.toLocaleString()], ['Total Compute Time', `${(report.totalDurationMs / 1000).toFixed(1)}s`], ['Active Agents', `${Object.keys(report.perAgent).length}`]);
    console.log(table.toString());
    // Per-agent breakdown
...
program
    ...
    configTable.push(['Anthropic (Claude)', config.hasAnthropic ? chalk.green('OK') : chalk.yellow('NOT SET'), 'ANTHROPIC_API_KEY'], ['Supabase DB', config.hasSupabase ? chalk.green('OK') : chalk.yellow('NOT SET'), 'SUPABASE_URL + SUPABASE_SERVICE_KEY'], ['Perplexity', config.hasPerplexity ? chalk.green('OK') : chalk.yellow('NOT SET'), 'PERPLEXITY_API_KEY'], ['Stripe', config.hasStripe ? chalk.green('OK') : chalk.yellow('NOT SET'), 'STRIPE_SECRET_KEY'], ['X / Twitter', config.hasX ? chalk.green('OK') : chalk.yellow('NOT SET'), 'X_API_KEY + X_ACCESS_TOKEN'], ['YouTube', config.hasYouTube ? chalk.green('OK') : chalk.yellow('NOT SET'), 'YOUTUBE_API_KEY'], ['TikTok', config.hasTikTok ? chalk.green('OK') : chalk.yellow('NOT SET'), 'TIKTOK_CLIENT_KEY + TIKTOK_CLIENT_SECRET']);
...
program
    ...
    const stubLogger = { info: console.log, warn: console.warn, error: console.error, debug: console.log, child: () => stubLogger };
    ...
    try {
        ...
        spin.fail(chalk.red(`Auth failed: ${err.message}`));
        process.exit(1);
    }
});
program.parse();
