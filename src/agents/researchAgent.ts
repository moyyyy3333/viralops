...
export class ResearchAgent implements Agent {
  ...
  async execute(input: unknown, context: AgentContext): Promise<ViralTopic[]> {
    const { niche, count, topics } = input as { niche: string; count: number; topics: string[] };
    ...
    let perplexityResult: Awaited<ReturnType<PerplexityClient['getTrendingTopics']>>;
    try {
      perplexityResult = await withRetry(
        () => this.perplexity.getTrendingTopics(effectiveNiche, desiredCount),
        { maxRetries: 3 },
        logger
      );
    } catch {
    ...
    for (let i = 0; i < perplexityResult.length; i++) {
      ...
      try {
        ...
        let tags: string[] = this.extractTags(topic.title);
        ...
        if (claude.complete) {
          try {
            ...
            const { data, tokensUsed, costMs } = await claude.parseJSON<{ viralScore: number; sentiment: string; tags: string[] }>(prompt);
            viralScore = Math.min(100, Math.max(1, Math.round(data.viralScore)));
            sentiment = ['positive', 'neutral', 'negative'].includes(data.sentiment) ? data.sentiment as 'positive' | 'neutral' | 'negative' : 'neutral';
        ...
        logger.warn(`Failed to analyze "${topic.title}": ${(err as Error).message}`);
      }
    }
    ...
    await context.db.logAgentEvent(context.runId, 'research', 'info',
      `Discovered ${viralTopics.length} viral topics`,
      { topics: viralTopics.map((t) => ({ title: t.title, score: t.viralScore })) }
    );

    return viralTopics;
  }

  private heuristicScore(title: string, views?: number): number {
    let score = 50;
    if (views && views > 0) score = Math.min(95, 30 + Math.log10(views) * 8);
    const boost = ['viral', 'trending', 'shocking', 'reveals', 'destroys', 'breaks', 'insane', 'mind-blowing'];
    for (const kw of boost) { if (title.toLowerCase().includes(kw)) { score += 8; break; } }
