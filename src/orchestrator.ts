...
import type {
  AgentType, PipelineRun, Task, TaskStatus,
  PipelineStatus, AgentContext, ViralTopic,
  VideoSegment, EditedVideo, PostResult,
} from './types.js';
...
export class PipelineOrchestrator {
  ...
  constructor() {
    ...
    for (const agentId of Object.keys(this.agents) as AgentType[]) {
  ...
  async runPipeline(options: {
    ...
    try {
      ...
      const topics = await this.executeAgent<'research', ViralTopic[]>('research', {
        niche,
        count: 5,
      ...
      const segments = await this.executeAgent<'clip', VideoSegment[]>('clip', {
      ...
      const editedVideos = await this.executeAgent<'edit', EditedVideo[]>('edit', {
        segments,
      ...
      const [postResults, financeReport] = await Promise.all([
        editedVideos && editedVideos.length > 0
          ? this.executeAgent<'post', PostResult[]>('post', {
              videos: editedVideos,
              dryRun: options.dryRun ?? false,
            }, runId)
          : Promise.resolve([]),
        this.executeAgent<'finance', any>('finance', {
          runId,
          platform: 'all',
        }, runId),
      ]);
  ...
  private async executeAgent<T extends AgentType, R>(
    agentId: T,
    ...
    try {
      const result = await withRetry(
        () => circuitBreaker.execute(() => this.agents[agentId].execute(input, context)),
        { maxRetries: config.defaultRetryCount },
        agentLogger
      );
  ...
  private getAgentPriority(agentId: AgentType): number {
  ...
  async getRecentLogs(runId?: string, limit = 50) {
    if (runId) {
