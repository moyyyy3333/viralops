...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === "/oauth2callback") {
    ...
    if (error) { res.writeHead(400).end("Error"); console.log(error); server.close(); process.exit(1); return; }
    ...
    try {
      ...
      } else { res.writeHead(400).end(JSON.stringify(tokens)); console.log('FAIL:', JSON.stringify(tokens)); }
    } catch (err) { res.writeHead(500).end(err.message); console.log('ERROR:', err.message); }
    finally { server.close(); setTimeout(() => process.exit(0), 2000); }
  } else { res.writeHead(200).end("<h3>Waiting...</h3>"); }
});
