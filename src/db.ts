...
export class SupabaseDB implements DBLike {
  ...
  async select(table: string, query?: Record<string, unknown>): Promise<Record<string, unknown>[]> {
    if (!this.enabled || !this.client) return [];
  ...
  async getRecentLogs(runId: string, limit = 50): Promise<Record<string, unknown>[]> {
    if (!this.enabled || !this.client) return [];
