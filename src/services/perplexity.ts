...
export class PerplexityClient {
  ...
  async search(query: string): Promise<{ content: string; sources: string[]; costMs: number }> {
    if (!this.enabled) {
    ...
    try {
      ...
      this.logger.error('Perplexity search failed', { error: (error as Error).message });
   ...
   */
  async getTrendingTopics(niche: string, count: number): Promise<ViralTopic[]> {
...
Joe Rogan Experience #2215 - Elon Musk Discusses AI | YouTube | 45M | https://www.youtube.com/watch?v=dQw4w9WgXcQ`;
   ...
   */
  private async fallbackYouTubeSearch(niche: string, count: number): Promise<ViralTopic[]> {
...
Format: TITLE | PLATFORM | VIEWS | URL`;
  ...
  private extractYouTubeUrls(citations: string[]): string[] {
    const urls: string[] = [];
    for (let c of citations) {
      // Strip trailing citation markers like [2], [3], etc.
      c = c.replace(/\[\d+\]$/, '');
      if (
        c.startsWith('https://www.youtube.com/watch?v=') ||
  ...
  private parsePipeResponse(content: string, sources: string[]): ViralTopic[] {
  ...
  private getSearchQueries(niche: string, count: number): string[] {
  ...
  private parseViews(val: string): number | undefined {
